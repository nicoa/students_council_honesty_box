...
parser.add_argument(
    "--csv_new",
    help="Specify new path for calculated csv file.",
    default="")
parser.add_argument(
    "--out", "--output_path",
...
if args.csv_new == '':
    csv_new = (lambda s: ".".join(s[:-1]) + '_copy.' + s[-1])(
        args.csv_path.split("."))
else:
...
cap_string_format = wb.add_format({
    'border': 1, 'valign': 'vcenter', 'align': 'center'})
eur_format = wb.add_format({
    'num_format':
        u'[>{}]#,##0.00 [$€-407];[<{}][RED]-#,##0.00 [$€-407]'.format(
            args.cap / 2, args.cap / 2),
    'bottom': 1, 'left': 2, 'right': 2, 'valign': 'vcenter', 'align': 'right'})
...
column_width = np.percentile(
    [len(unicode(valu)) for valu in schulden['name']], 80)
...
for i, df_entry in schulden.iterrows():
    ...
    if df_entry['val'] < args.cap:
        ...
        ws.merge_range(
            ...
            c['cap_format_str'].format(args.cap), cap_string_format)
    else:
